...
#include <rt/integrators/montecarlo.h>
...
void a_instancing() {
   ...
   Group* scene = new SAHKDTree();

    Instance* normal = new Instance(tree);
	normal->translate(Vector(0.0f, 0.0f, 3.0f));
	scene->add(normal);
	...
	//MonteCarloIntegrator integrator(&world);
    PerspectiveCamera cam(Point(-3.75f, 20, 40), Vector(0.1f, -0.5f, -1), Vector(0, 1, 0), pi/4, pi/3);
	//PerspectiveCamera cam(Point(0.0f, 1.0f, -10.0f), Vector(0.0f, 0.2f, 1.0f), Vector(0.0f, 1.0f, 0.0f), pi / 4, pi / 3);
